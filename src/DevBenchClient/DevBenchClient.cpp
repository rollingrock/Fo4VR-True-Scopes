...
namespace DevBenchClient
{
	namespace
	{
		...
		//
		// devbench hands us the tool's arguments as a JSON string. This plugin has no
		// JSON library on purpose (see DevBench.h), and pulling one in to read three
		// scalar fields would be the tail wagging the dog.
		//
		// So: extract top-level scalars only, and REFUSE anything else rather than
		// guess. Two properties make that safe rather than reckless:
		//   * the input is generated by devbench from a JSON Schema we publish below,
		//     so it is well-formed and flat by construction;
		//   * depth is tracked, so a key inside a nested object or array can never be
		//     mistaken for the top-level key of the same name.
		// A malformed or unexpected input yields "not found", which every caller turns
		// into a stated default or an error — never a silent wrong value.
		...
		// --- the tool ------------------------------------------------------------
		//
		// ONE tool with an `action`, not nine tools. devbench's own guidance is to keep
		// the agent-facing surface small, and an agent choosing between `scope_state`,
		// `scope_config`, `scope_omods`... is worse at it than one choosing an enum.
		//
		// Every action maps to a route on our own :8930 server and is answered by
		// DevBench::Invoke — the SAME handler the HTTP listener runs. Nothing is
		// reimplemented here, so the two transports cannot give different answers while
		// both are up. When :8930 is retired the routes stay; only the listener goes.
		void ScopeTool(void*, const char* a_argsJson, void* a_sink, DevBenchAPI::WriteFn a_write)
		{
			...
			if (action == "state") {
				path = "/scope";
				// `probe` forces a fresh 3D walk. ⚠ IT MUTATES: it calls
				// InvalidatePlacement, which recomputes the widget placement. That is
				// not a footnote — on 2026-08-11 it produced a whole day of false
				// confirmations because every measurement repaired what it measured.
				// Default OFF here, deliberately, so the ordinary read observes.
				std::string probe;
  ...
  "timeoutMs":{"type":"integer","description":"action=dump: how long to wait for the next render (default 5000)."}
	...
	void Register()
	{
		...
		if (!api) {
			...
			// requirement — the :8930 listener is unaffected either way.
